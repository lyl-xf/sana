...
public class MessageClientListener {
    ...
    public void thingSubRegister() {
        ...
        String subTopic = generalPrefix.getProxyPrefix()+"#";
        log.info("开始初始MQTT订阅设备数据主题:{}",subTopic);
        client.subQos0(subTopic, (context, topic, message, payload) -> {
            try {

                if ("REDIS".equalsIgnoreCase(queueType)) {
                    //拼接测试数据
                    HashMap<String, Object> streamMap = new HashMap<>(2);
                    streamMap.put("deviceId", topic.substring(generalPrefix.getProxyPrefix().length()));
                    ...
                    Object timingData = redisCacheOps.get(CacheKeyBuilder.deviceIdRuleJon(generalPrefix.getTablePrefix() + topic.substring(generalPrefix.getProxyPrefix().length())));
                    // 生成随机数决定发送到哪个流
