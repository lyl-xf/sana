			...
			<div class="sc-user-select">
				<div class="sc-user-select__left">
					<div class="sc-user-select__search">
						<el-input v-model="keyword" prefix-icon="el-icon-search" placeholder="搜索设备">
							<template #append>
	...
	methods: {
		...
		open(type, data){
			...
			if(this.type==1){
				//设备控制
				this.getDeviceList()
				this.getDeviceItemPage()
			}else if(this.type==2){
				//数据流转
				this.getDataPortering()
			}else if(this.type==3){
				this.getProtocol()
			}
		...
		async getProtocol(){
			this.showGrouploading = true;
			var res = await config.protocol.apiObj.get();
			this.showGrouploading = false;
			this.role = config.role.parseData(res).rows
		},
		//组织点击
